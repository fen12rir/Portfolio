...
import { uploadImageFromDataUrl, getConfiguredImageProvider } from '../server/utils/imageStorage.js';
...
const uploadLimiter = createRateLimiter({
  keyPrefix: 'portfolio-upload-api',
  windowMs: 60 * 1000,
  maxRequests: 20,
  message: 'Too many image uploads. Please wait a moment.',
});

router.post('/upload-image', requireAdminAuth, uploadLimiter, asyncHandler(async (req, res) => {
  try {
...
// Save portfolio data
router.post('/', requireAdminAuth, writeLimiter, asyncHandler(async (req, res) => {
  try {
    if (!process.env.MONGODB_URI) {
      return res.status(503).json({ 
        success: false,
...
router.post('/cleanup', requireAdminAuth, writeLimiter, asyncHandler(async (req, res) => {
  try {
    if (!process.env.MONGODB_URI) {
      return res.status(503).json({ 
        success: false,
...
router.delete('/', requireAdminAuth, writeLimiter, asyncHandler(async (req, res) => {
  try {
    if (!process.env.MONGODB_URI) {
      return res.status(503).json({ 
        success: false,
